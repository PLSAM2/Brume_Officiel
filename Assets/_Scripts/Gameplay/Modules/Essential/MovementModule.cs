...
public class MovementModule : MonoBehaviour
{
	...
	{
		if (CastSphereAll(_direction, _layerTocheck, _maxRange, transform.position) != null && CastSphereAll(_direction, _layerTocheck, _maxRange, transform.position).Count > 0)
			return false;
		else
			return true;
	}
	...
	{
		List<RaycastHit> _allHit = new List<RaycastHit>();
		_allHit.AddRange(Physics.SphereCastAll(_origin,
			chara.radius,
			_direction,
			_maxRange,
